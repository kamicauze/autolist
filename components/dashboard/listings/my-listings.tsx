...
const listingStatusOptions = [
  "all",
  ...Object.keys(LISTING_STATUS_META),
] as const;
...
function normalizeStatusLabel(value: string) {
  ...
  return (
    LISTING_STATUS_META[value as keyof typeof LISTING_STATUS_META]?.label ??
    value.replace(/_/g, " ")
  );
}
...
function formatListingPrice(listing: Listing) {
  ...
  return listing.currency?.toUpperCase() === "KES"
    ? formatted.replace(/^KES\s*/i, "Ksh")
    : formatted;
}
  ...
  switch (status) {
    case "active":
      return {
        label: "Approved",
        className:
          "border-[rgba(126,211,33,0.14)] bg-[rgba(126,211,33,0.1)] text-[#7ed321]",
      };
    case "pending":
      return {
        label: "Pending",
                ...
                >
                  ...
                  {salesReps.map((rep) => (
                    <DropdownMenu.Item
                      ...
                      disabled={
                        isPending || listing.assigned_agent_id === rep.id
                      }
                      className={itemClass}
                    >
...
function BidsPlaceholder({
  ...
  const handleMakeOffer = async (row: OfferRow) => {
    ...
    const message =
      window.prompt("Add a short message for the seller", "") || "";
  ...
  const handleAcceptOffer = async (row: OfferRow) => {
    const confirmed = window.confirm(
      "Accept this offer? This will reserve the listing and reject competing live offers.",
    );
    if (!confirmed) return;
    ...
    return (
      <div className="space-y-3 px-5 py-5">
        {[1, 2, 3].map((item) => (
          <div
            key={item}
            className="h-16 animate-pulse rounded-[8px] bg-[#f1f5f9]"
          />
        ))}
      </div>
    );
  ...
  if (mode === "available") {
    ...
    return (
      <div className="space-y-8 px-0 pb-8">
        <div className="flex justify-end px-0 pt-1">
        ...
        {bidRows.length === 0 ? (
          <div className="rounded-[12px] border border-dashed border-[#d1d5db] bg-[#f9fafb] px-6 py-16 text-center">
            <p className="text-[22px] font-bold text-[#101828]">
              No available bids yet
            </p>
            <p className="mx-auto mt-2 max-w-xl text-[14px] leading-6 text-[#4a5565]">
              Dealer bid opportunities will appear here as cards when seller
              listings are open for offers.
            </p>
          </div>
        ) : (
                  ...
                  <div className="flex flex-col gap-5">
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-[14px] leading-[21px] text-[#4a5565]">
                            Submitted by {offer.buyerName || "Private seller"}
                          </p>
                        </div>
                      </div>
                    ...
                    <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-4">
                      ...
                      <BidMetric
                        ...
                        value={
                          offer.amount
                            ? formatDashboardCurrency(offer.amount)
                            : "Not provided"
                        }
                      />
                    </div>
                    ...
                    {offer.description ? (
                      <p className="whitespace-pre-line text-[14px] leading-[22.75px] text-[#364153]">
                        {offer.description}
                      </p>
                    ) : null}
                    ...
                    <div className="flex flex-col gap-4 border-t border-[#e5e7eb] pt-5 lg:flex-row lg:items-center lg:justify-between">
                      <div className="space-y-1">
                        ...
                        <p className="text-[12px] leading-[18px] text-[#6a7282]">
                          {offer.createdAt
                            ? `Submitted on ${formatDashboardDate(offer.createdAt)}`
          ...
          {rows.map((offer) => (
            <tr key={offer.id} className="text-[14px] text-[#202224]">
              <td className="px-5 py-4 font-semibold">
                {offer.listingTitle || "Listing offer"}
              </td>
              ...
              <td className="px-5 py-4 text-[#5f6673]">
                {offer.createdAt
                  ? formatDashboardDate(offer.createdAt)
                  : "Not available"}
              </td>
...
function BidMetric({
  ...
  return (
    ...
    >
      ...
      <p
        className={cn(
          "mt-2 text-[18px] font-bold leading-[27px]",
          highlight ? "text-[#1c398e]" : "text-[#101828]",
        )}
      >
        {value}
      </p>
...
export function MyListings({
  ...
  const handleDeleteListing = (listing: Listing) => {
    ...
    const confirmed = window.confirm(
      `Delete "${title}"? This will remove it from seller and public views.`,
    );
    if (!confirmed) return;
  ...
  const handleBulkDelete = async () => {
    const confirmed = window.confirm(
      `Delete ${selectedIds.length} selected listing(s)?`,
    );
    if (!confirmed) return;

    const idsToDelete = [...selectedIds];
    setDeletingIds((current) =>
      Array.from(new Set([...current, ...idsToDelete])),
    );
  ...
  const handleBulkArchive = async () => {
    const confirmed = window.confirm(
      `Archive ${selectedIds.length} selected listing(s)?`,
    );
    if (!confirmed) return;

    const idsToArchive = [...selectedIds];
    setPendingIds((current) =>
      Array.from(new Set([...current, ...idsToArchive])),
    );
    ...
    setPendingIds((current) =>
      current.filter((id) => !idsToArchive.includes(id)),
    );
    setSelectedIds([]);
    router.refresh();
  };
  ...
  const handleHold = (listing: Listing) => {
    ...
    void runListingAction(listing, () =>
      updateOwnerListingStatus(listing.id, nextStatus),
    );
  };
    ...
    void assignListingToSalesRep(listing.id, agentId).then((result) => {
      if ("error" in result) {
        ...
        setListingItems((current) =>
          current.map((item) =>
            item.id === listing.id
              ? { ...item, assigned_agent_id: previous }
              : item,
          ),
        );
        return;
