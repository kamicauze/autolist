...
export async function POST(
  ...
  if (listing.status !== "active") {
    return NextResponse.json({ error: "Only active listings can receive enquiries." }, { status: 400 });
  }
  ...
  if (threadError || !thread) {
    return NextResponse.json({ error: threadError?.message || "Unable to open conversation thread." }, { status: 400 });
  }
