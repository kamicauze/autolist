...
create table public.dealers (
  ...
  preferred_confirmation_channel text default 'email' check (preferred_confirmation_channel in ('email', 'whatsapp', 'sms')),
...
create policy "Admins view all dealer profiles." on public.dealers for select using (
  exists (select 1 from public.profiles where id = auth.uid() and role = 'admin')
);
create policy "Admins update dealer profiles." on public.dealers for update using (
  exists (select 1 from public.profiles where id = auth.uid() and role = 'admin')
);

create table public.dealer_verification_documents (
  id uuid default uuid_generate_v4() primary key,
  dealer_id uuid references public.dealers(id) on delete cascade not null,
  uploaded_by uuid references public.profiles(id) not null,
  document_type text not null check (document_type in ('company_logo', 'contact_photo', 'incorporation_certificate', 'contact_id')),
...
create policy "Owners view own dealer verification documents." on public.dealer_verification_documents for select using (
  exists (select 1 from public.dealers where dealers.id = dealer_verification_documents.dealer_id and dealers.profile_id = auth.uid())
);
create policy "Owners insert own dealer verification documents." on public.dealer_verification_documents for insert with check (
  uploaded_by = auth.uid()
  and exists (select 1 from public.dealers where dealers.id = dealer_verification_documents.dealer_id and dealers.profile_id = auth.uid())
);
create policy "Owners delete own dealer verification documents." on public.dealer_verification_documents for delete using (
  exists (select 1 from public.dealers where dealers.id = dealer_verification_documents.dealer_id and dealers.profile_id = auth.uid())
);
create policy "Admins view all dealer verification documents." on public.dealer_verification_documents for select using (
  exists (select 1 from public.profiles where id = auth.uid() and role = 'admin')
);
